...
for (c in 1:length(celltype)) {
  ...
  for (i in 1:length(seu_list)) {
    ...
    entropy <- compute_entropy(counts,
                               unit = "log2",
                               method_entropy = "KL.plugin", # Now method can select one of KL.plugin(Default), ChaoShen, Dirichlet or entropy
                               compute_var = TRUE,
                               normalise = TRUE,
